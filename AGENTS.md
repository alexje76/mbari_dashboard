...
- `power.html` (`js/pages/power.js`) plots Battery %, a merged Power chart (per-controller Avg Power solid, Power In dashed, Discharge solid-bold = `power_to_controller` + a 0–100 W unmeasured-discharge slider default 15 W with a tick at 15, and a thin-dashed low-opacity `Phantom (no offset)` line = measured discharge without the slider), and Battery Life (hours = `BATTERY_CAPACITY_WH` × pct ÷ net, gap while charging). Battery Life also overlays a flat dashed line per controller for expected life over that controller's most recent contiguous run (`BATTERY_CAPACITY_WH` × avg pct ÷ avg net), plus a text summary; the summary and projection lines recompute on slider moves. `BATTERY_CAPACITY_WH = 5000` constant, swappable.
