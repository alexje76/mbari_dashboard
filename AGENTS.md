...
- `selector.js` gates timeline rows on the sea-state selector; when a dataset has no Hs/Tp columns (e.g. real-pipeline output before the wave loader lands) it falls back to showing every row and shows a "No sea-state data in range" note with the scatter toggle disabled. The sea-state filter re-engages automatically once any Hs/Tp point exists.
- `nextwave.js` renders one stacked chart per `chartTypes.json` entry in the `prediction` category (`nextwave`, `nextwave_error`, `nextwave_error_2`; new prediction columns auto-appear). Selection persists via the `chartTypes` URL param; the `nextwave` state column uses a categorical y-axis, error columns numeric lines; stacked charts share synced zoom. It also renders a Prediction Scatter (`#chartScatter`) with selectable X (any numeric chartTypes metric) and Y (`nextwave_error`/`nextwave_error_2`/`nextwave` state → axis positions Off=1/Starting=2/On=3) axes; a whole-page controller selector (`controllers` URL param) filters which rows feed both the stacked charts and the scatter. Axis/metric selections persist via `scatterX`/`scatterY` URL params.
...
- **Reference dashboard:** Sandia Pioneer WEC (https://sandialabs.github.io/pioneer_wec_dashboard/, source `sandialabs/pioneer_wec_dashboard`) — a different WEC program's static Plotly/Jekyll dashboard with sea-state/power-capture analytics. See "Future Considerations & Notes" in `architecture_decisions (4).md` for a full note and flagged visualization ideas.
...
- `power.html` (`js/pages/power.js`) plots Battery %, a merged Power chart (per-controller Avg Power solid + signed, Power In dashed + positive, Discharge solid-bold = `-(power_to_controller + slider)` and a thin-dashed low-opacity `Phantom (no offset)` line = `-power_to_controller`, both plated as negative draws per the signed-power convention; slider is 0–100 W unmeasured discharge, default 15 W with a tick at 15), and Battery Life (hours = `BATTERY_CAPACITY_WH` × pct ÷ net, gap while charging). Axis-trigger tooltips list only the series present at the hovered point (timestamp only when none). Battery Life also overlays a flat dashed line per controller for expected life over that controller's most recent contiguous run (`BATTERY_CAPACITY_WH` × avg pct ÷ avg net), plus a text summary; the summary and projection lines recompute on slider moves. `BATTERY_CAPACITY_WH = 5000` constant, swappable.
